...
const onCalculators = [
  { slug: "termination-pay", name: "Termination Pay Calculator", description: "Calculate Ontario termination pay entitlements based on length of service.", icon: UserMinusIcon },
  { slug: "severance-pay", name: "Severance Pay Calculator", description: "Determine Ontario severance pay obligations.", icon: UsersIcon },
  { slug: "overtime", name: "Overtime Pay Calculator", description: "Calculate weekly overtime pay under Ontario law.", icon: ClockIcon },
  { slug: "public-holiday", name: "Public Holiday Pay Calculator", description: "Calculate public holiday pay eligibility and entitlements in Ontario.", icon: StarIcon },
  { slug: "vacation-pay", name: "Vacation Pay Calculator", description: "Calculate Ontario vacation pay and time entitlement.", icon: SunIcon },
  { slug: "minimum-wage", name: "Minimum Wage Calculator", description: "Check if a wage meets the current Ontario minimum wage requirement.", icon: CurrencyDollarIcon },
  { slug: "average-weekly-earnings", name: "Average Weekly Earnings Calculator", description: "Calculate average weekly earnings for Ontario employment claims.", icon: ChartBarIcon },
  { slug: "pregnancy-parental-leave", name: "Pregnancy & Parental Leave Calculator", description: "Calculate Ontario leave entitlements and estimate EI benefits.", icon: HeartIcon },
];

const theme = getTheme("ca", "on");

function getRuleSummary(slug: string): string | null {
  const rule = getRuleFromBundle("on", slug);
  if (!rule) return null;
  ...
  switch (slug) {
    ...
    case "severance-pay":
      return `5+ years required. Up to 26 weeks. Payroll or group termination threshold.`;
    case "overtime":
      return `1.5x after 44h weekly. No general daily threshold.`;
    case "public-holiday":
      return `Must work before/after holiday. Average daily wage applies.`;
    case "vacation-pay":
      return `4% (<5 yrs), 6% (5+ yrs). 2–3 weeks time.`;
    case "minimum-wage":
      return `Current: $${data.generalMinimumWage ?? 17.20}/hr general minimum.`;
    case "average-weekly-earnings":
      return `Used for termination pay and other ESA calculations.`;
    case "pregnancy-parental-leave":
      return `17 weeks pregnancy + up to 61 weeks parental (ON ESA).`;
    default:
      return null;
  }
}
...
function getApplicableTemplates(templates: DocumentTemplate[]): DocumentTemplate[] {
  return templates.filter((t) => {
    ...
    return t.jurisdictionScopes.some((s) => s.toLowerCase() === "ca-on" || s.toLowerCase() === "on");
  });
}

export async function generateMetadata() {
  return {
    title: "Ontario Legal Calculators | LegalCals",
    description: "Browse province-specific legal calculators for Ontario. Calculate termination pay, severance pay, overtime, public holiday pay, vacation pay, minimum wage, and more.",
  };
}
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <div className="px-4 py-12 max-w-5xl mx-auto space-y-12">
        <StateSectionHeader
          title="Legal Calculators for Ontario"
          description="Browse jurisdiction-specific calculators, rule summaries, and document templates for Ontario."
          icon={MapIcon}
          theme={theme}
        />
        ...
        <section className="space-y-6">
          <StateSectionHeader
            ...
            description="Select a calculator to run it with Ontario-specific rules."
            icon={CalculatorIcon}
            theme={theme}
          />

          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {onCalculators.map((calc) => {
              const Icon = calc.icon || CalculatorIcon;
              const summary = getRuleSummary(calc.slug);

              return (
                <Link key={calc.slug} href={`/calculators/ca/on/${calc.slug}`}>
                  <LCCard
                    theme={theme}
        ...
        {templates.length > 0 && (
          <section className="space-y-6">
            <StateSectionHeader
              ...
              description="Auto-generate Ontario-specific legal documents."
...
></div>`}</code>
        ...
        <section className="space-y-6">
          <StateSectionHeader
            title="API Access"
            description="Query Ontario rules programmatically."
          ...
          <LCCard theme={theme} className="space-y-4">
            ...
            <div>
              ...
              <div className="bg-slate-900 text-slate-100 rounded-lg p-4 overflow-x-auto">
                <pre className="text-sm font-mono">
                  <code>{`GET /api/authority/ca/on/{topic}`}</code>
                </pre>
              </div>
            </div>
            <div>
  ...
  https://legalcals.com/api/authority/ca/on/termination-pay`}</code>
                </pre>
              </div>
            </div>
        ...
        <section>
          <LCCard theme={theme} className="space-y-3">
            <p className="text-sm text-slate-700 leading-relaxed">
              Need more details on Ontario employment standards? Visit the{" "}
              <Link href="/ca/provinces/on" className="underline font-medium" style={{ color: theme.colors.primary }}>
                Ontario Province Hub
              </Link>{" "}
