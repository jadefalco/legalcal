...
export default function ONVacationPayCalculator() {
  ...
  const rule = getRuleFromBundle("on", "vacation-pay");
  ...
  const extendedThreshold =
    typeof data?.extendedThresholdYears === "number"
      ...
      : 5;
  const baseWeeks =
    typeof data?.baseVacationWeeks === "number" ? data.baseVacationWeeks : 2;
  const extendedWeeks =
    typeof data?.extendedVacationWeeks === "number"
      ? data.extendedVacationWeeks
      : 3;
  ...
  function handleCalculate() {
    ...
    const weeks = years >= extendedThreshold ? extendedWeeks : baseWeeks;
  ...
  return (
    <div className="space-y-8">
      <LCSection
        title="ON Vacation Pay Calculator"
        description="Calculate vacation pay entitlements under the Ontario Employment Standards Act."
        icon={SunIcon}
        theme={theme}
      />
      ...
      <LCCard theme={theme} className="space-y-5">
        <p className="text-sm text-slate-600">
          Enter your total wages and years of service to calculate your Ontario
      ...
      <LCCard theme={theme} className="space-y-3">
        ...
        <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
          ...
          <li>{baseWeeks} weeks vacation time ({extendedWeeks} weeks after {extendedThreshold} years)</li>
          <li>Based on Ontario ESA §33–§42</li>
        </ul>
      </LCCard>
    </div>
  );
