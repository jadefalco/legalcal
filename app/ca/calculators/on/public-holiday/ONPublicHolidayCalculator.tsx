...
export default function ONPublicHolidayCalculator() {
  ...
  const rule = getRuleFromBundle("on", "public-holiday");
  ...
  return (
    <div className="space-y-8">
      <LCSection
        title="ON Public Holiday Pay Calculator"
        description="Calculate public holiday pay eligibility and entitlements under the Ontario Employment Standards Act."
        icon={StarIcon}
        theme={theme}
      />
      ...
      <LCCard theme={theme} className="space-y-5">
        <p className="text-sm text-slate-600">
          ...
          details to calculate Ontario public holiday pay.
        </p>
          ...
          {!result.eligible && (
            <div className="p-3 bg-amber-50 rounded-lg flex items-start gap-2">
              ...
              <p className="text-sm text-amber-800">
                Not eligible for public holiday pay. You must work the scheduled
                day before and after the holiday (unless reasonable cause for
                absence).
              </p>
            </div>
          )}
      ...
      <LCCard theme={theme} className="space-y-3">
        <h4 className="font-semibold text-slate-800 text-sm flex items-center gap-2">
          <InformationCircleIcon className="w-4 h-4 text-blue-500" />
          About Ontario Public Holiday Pay
        </h4>
        <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
          <li>
            Must work the scheduled day before and after the holiday (unless
            reasonable cause)
          </li>
          <li>
            Public holiday pay = total regular wages + vacation pay in 4 weeks
            before holiday ÷ {divisorDays}
          </li>
          <li>If worked: 1.5x regular wage + public holiday pay</li>
          <li>If not worked: public holiday pay</li>
          <li>Based on Ontario ESA §24–§32</li>
        </ul>
      </LCCard>
    </div>
  );
