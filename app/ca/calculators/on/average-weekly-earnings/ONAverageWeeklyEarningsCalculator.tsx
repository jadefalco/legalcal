...
export default function ONAverageWeeklyEarningsCalculator() {
  ...
  const rule = getRuleFromBundle("on", "average-weekly-earnings");
  ...
  return (
    <div className="space-y-8">
      <LCSection
        title="ON Average Weekly Earnings Calculator"
        description="Calculate average weekly earnings for Ontario employment standards claims and entitlements."
        icon={ChartBarIcon}
        theme={theme}
      />
      ...
      <LCCard theme={theme} className="space-y-5">
        <p className="text-sm text-slate-600">
          ...
          weekly earnings under Ontario rules.
        </p>
      ...
      <LCCard theme={theme} className="space-y-3">
        <h4 className="font-semibold text-slate-800 text-sm flex items-center gap-2">
          <InformationCircleIcon className="w-4 h-4 text-blue-500" />
          About Ontario Average Weekly Earnings
        </h4>
        <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
          <li>Used for calculating termination pay and other entitlements</li>
          ...
          <li>Based on Ontario ESA §1 and related regulations</li>
        </ul>
      </LCCard>
    </div>
  );
