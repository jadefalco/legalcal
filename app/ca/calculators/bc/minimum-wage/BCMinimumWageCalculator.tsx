...
export default function BCMinimumWageCalculator() {
  ...
  const rule = getRuleFromBundle("bc", "minimum-wage");
  ...
  return (
    <div className="space-y-8">
      <LCSection
        title="BC Minimum Wage Calculator"
        description="Check if a wage meets the current BC minimum wage requirement."
      ...
      <LCCard theme={theme} className="space-y-5">
        <p className="text-sm text-slate-600">
          Enter an hourly wage to verify compliance with the current BC minimum
          wage rate.
        </p>
            ...
            <>
              ...
              <div className="p-3 bg-amber-50 rounded-lg flex items-start gap-2">
                ...
                <p className="text-sm text-amber-800">
                  This wage is below the BC minimum. Employers must pay at least
                  ${result.currentMinimum.toFixed(2)} per hour for most
                  occupations.
                </p>
              </div>
            </>
          ...
          {result.meetsMinimum && (
            <div className="p-3 bg-green-50 rounded-lg flex items-start gap-2">
              ...
              <p className="text-sm text-green-800">
                This wage meets or exceeds the current BC minimum wage of ${result.currentMinimum.toFixed(2)} per hour.
              </p>
            </div>
          )}
        </LCCard>
      ...
      <LCCard theme={theme} className="space-y-3">
        <h4 className="font-semibold text-slate-800 text-sm flex items-center gap-2">
          <InformationCircleIcon className="w-4 h-4 text-blue-500" />
          About BC Minimum Wage
        </h4>
        <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
          <li>General minimum wage: ${currentMinimum.toFixed(2)}/hour</li>
          <li>
            Some occupations (e.g., liquor servers, live-in camp leaders) have
            different rates
          </li>
          <li>Based on BC Employment Standards Act §8, §16</li>
        </ul>
      </LCCard>
    </div>
  );
