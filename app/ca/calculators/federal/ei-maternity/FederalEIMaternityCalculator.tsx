...
export default function FederalEIMaternityCalculator() {
  ...
  const rule = getRuleFromBundle("federal", "ei-maternity");
  ...
  const maxWeeks =
    typeof data?.maxWeeks === "number" ? data.maxWeeks : 15;
  ...
  function handleCalculate() {
    ...
    if (weeks < minWeeksRequired) {
      setError(
        `You must have worked at least ${minWeeksRequired} insurable weeks to qualify for EI maternity benefits.`
      );
      setResult(null);
      return;
    }
  ...
  return (
    <div className="space-y-8">
      <LCSection
        title="Federal EI Maternity Benefits Calculator"
        description="Calculate Employment Insurance maternity benefit entitlements under federal law."
        icon={HeartIcon}
        theme={theme}
      />
          ...
          {result.weeklyBenefit === maxWeeklyBenefit && (
            <div className="p-3 bg-amber-50 rounded-lg flex items-start gap-2">
              ...
              <p className="text-sm text-amber-800">
                Maximum weekly benefit reached. EI maternity benefits are capped
