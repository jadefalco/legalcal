...
export default function BCRentReceiptPage() {
  ...
  const [paymentMethodOtherDescription, setPaymentMethodOtherDescription] =
    useState("");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (paymentPeriodStart && paymentPeriodEnd) {
      ...
      if (!valid) {
        setError(
          "The payment period end date must be after the start date."
        );
        setLoading(false);
        return;
      }
    ...
    const payload: Record<string, unknown> = {
      ...
      paymentDate,
      paymentMethod,
      paymentMethodOtherDescription:
        paymentMethod === "other"
          ? paymentMethodOtherDescription || undefined
          : undefined,
      paymentPeriodStart,
      paymentPeriodEnd,
    ...
    try {
      const res = await fetch("/api/checkout/bc-rent-receipt", {
        method: "POST",
  ...
  return (
    <main className="min-h-screen px-4 py-10 max-w-4xl mx-auto">
      <div className="space-y-8">
        <LCSection
          title="BC Rent Receipt"
          description="Generate a compliant rent receipt under the British Columbia Residential Tenancy Act."
        ...
        <LCCard theme={theme} className="space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-200">
              ...
              <LCField label="Rent Amount (CAD)" theme={theme}>
                <input
                  type="number"
                  min={0}
                  step={0.01}
              ...
              <LCField label="Payment Date" theme={theme}>
                <input
                  type="date"
                  required
                  value={paymentDate}
              ...
              <LCField label="Payment Method" theme={theme}>
                <select
                  required
                  value={paymentMethod}
              ...
              {paymentMethod === "other" && (
                <LCField label="Other Payment Method Description" theme={theme}>
                  <input
                    type="text"
                    required
                    value={paymentMethodOtherDescription}
                    onChange={(e) =>
                      setPaymentMethodOtherDescription(e.target.value)
                    }
                    className={inputClass}
                  />
                </LCField>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <LCField label="Payment Period Start" theme={theme}>
                  <input
                    type="date"
                    required
                ...
                <LCField label="Payment Period End" theme={theme}>
                  <input
                    type="date"
                    required
              ...
              <LCField label="Receipt Notes (Optional)" theme={theme}>
                <textarea
                  rows={3}
                  value={receiptNotes}
            ...
            <div className="pt-4">
              ...
              >
                {loading ? "Processing..." : "Generate Receipt — $3 CAD"}
              </LCButton>
            </div>
          </form>
        </LCCard>
