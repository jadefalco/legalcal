...
export default function BCNoticeOfEntryPage() {
  ...
  const [entryReasonOtherDescription, setEntryReasonOtherDescription] =
    useState("");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (entryReason && noticeServeDate && entryDate) {
      const { valid } = validateEntryNotice(
        entryReason,
        noticeServeDate,
        entryDate
      );
      if (!valid) {
        setError(
          "BC law requires at least 24 hours’ written notice for this type of entry."
        );
        setLoading(false);
        return;
      }
    ...
    const payload: Record<string, unknown> = {
      ...
      rentalPostalCode,
      entryReason,
      entryReasonOtherDescription:
        entryReason === "other" ? entryReasonOtherDescription || undefined : undefined,
      entryDate,
      entryTime,
      noticeServeDate,
    };
    ...
    try {
      const res = await fetch("/api/checkout/bc-notice-of-entry", {
        method: "POST",
  ...
  return (
    <main className="min-h-screen px-4 py-10 max-w-4xl mx-auto">
      <div className="space-y-8">
        <LCSection
          title="BC Notice of Entry"
          description="Generate a compliant notice of entry under the British Columbia Residential Tenancy Act."
        ...
        <LCCard theme={theme} className="space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-200">
              <h3 className="font-semibold text-slate-800">Entry Details</h3>
              <LCField label="Reason for Entry" theme={theme}>
                ...
                >
                  ...
                  <option value="showing_to_prospective_tenants">
                    Showing to prospective tenants
                  </option>
                  <option value="showing_to_prospective_buyers">
              ...
              {entryReason === "other" && (
                <LCField label="Other Reason Description" theme={theme}>
                  <input
                    type="text"
                    required
                    value={entryReasonOtherDescription}
                    onChange={(e) =>
                      setEntryReasonOtherDescription(e.target.value)
                    }
                    className={inputClass}
                  />
                </LCField>
