    ...
    async function load() {
      try {
        ...
        const sessionRes = await fetch(
          `/api/checkout/bc-notice-landlord/session?token=${token}`
        );
        ...
        const pdfRes = await fetch(
          `/api/generate/bc-notice-landlord?token=${token}`
        );
        if (!pdfRes.ok) {
        ...
        a.href = url;
        a.download = "bc-notice-to-end-tenancy.pdf";
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);

        // Convert to base64 for lawyer review
    ...
    <main className="min-h-screen px-4 py-10 max-w-4xl mx-auto">
      <div className="space-y-8">
        <LCSection
          ...
          description="Your BC Notice to End Tenancy has been generated and downloaded."
