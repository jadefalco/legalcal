"use client";

import { useState, useEffect } from "react";
import { getTheme } from "@/app/theme";
import { usStates } from "@/app/config/usStates";
import { LCSection } from "@/app/components/lc/LCSection";
import { CalculatorIcon } from "@heroicons/react/24/outline";

import { calculateRentReceipt } from "./logic";
import { RentReceiptForm } from "./ui";
import type { RentReceiptResult } from "./schema";

interface RentReceiptCalculatorProps {
  initialState?: string;
}

export default function RentReceiptCalculator({
  initialState,
}: RentReceiptCalculatorProps) {
  const [stateCode, setStateCode] = useState<string>(
    initialState?.toLowerCase() || "ca"
  );
  const [result, setResult] = useState<RentReceiptResult | null>(null);

  const stateInfo = usStates[stateCode as keyof typeof usStates];
  const theme = getTheme("us", stateCode);

  useEffect(() => {
    if (
      initialState &&
      usStates[initialState.toLowerCase() as keyof typeof usStates]
    ) {
      setStateCode(initialState.toLowerCase());
      setResult(null);
    }
  }, [initialState]);

  const handleCalculate = () => {
    const calcResult = calculateRentReceipt(stateCode);
    setResult(calcResult);
  };

  const stateName = stateInfo?.name || stateCode.toUpperCase();

  return (
    <div className="space-y-8">
      <LCSection
        title="Rent Receipt Requirements"
        description={`Check whether landlords in ${stateName} must provide rent receipts and under what conditions.`}
        icon={CalculatorIcon}
        theme={theme}
      />

      <RentReceiptForm
        theme={theme}
        stateName={stateName}
        onCalculate={handleCalculate}
        result={result}
      />
    </div>
  );
}
