/**
 * US Lease Termination Rules Page Generator
 *
 * Outputs: app/us/states/{code}/lease-termination/page.tsx (50 states)
 */
...
const DATASET_EXPORT = "leaseTerminationRules";
const SCHEMA_PATH = "app/validation/LeaseTerminationRulesSchema.ts";

const compiledPaths = [
  path.join(root, ".next", "server", "app", "data", "us", "leaseTerminationRules.js"),
  path.join(root, ".next", "server", "app", "data", "us", "leaseTerminationRules.mjs"),
];

const sourcePaths = [
  path.join(root, "app", "data", "us", "leaseTerminationRules.js"),
  path.join(root, "app", "data", "us", "leaseTerminationRules.mjs"),
];
...
export default function Page() {
  ...
  return (
    <main className="min-h-screen px-4 py-12 max-w-4xl mx-auto space-y-12">
      <LCSection
        title={"${stateName} Lease Termination Rules"}
...
async function run() {
  logStart("US Lease Termination Page Generator");

  const module = await loadDataset(compiledPaths, sourcePaths, DATASET_NAME);
  /** @type {import("../app/types/LeaseTerminationRules").LeaseTerminationRulesMap} */
  ...
  for (const [stateCode, rules] of Object.entries(data)) {
    const stateDir = path.join(OUTPUT_DIR, stateCode, "lease-termination");
  ...
  logSuccess("All US lease termination pages generated", count);
}
